...
/** Resets the display so that only the scaffold appears on the screen */
	public void reset() {
		drawScaffold(); //draw scaffold when the program restarts
	}
 ...
 */
	public void displayWord(String word) {
		//adds the label under the scaffold, with the correctly guessed letters
		double x = getWidth()/4;
		double y = getHeight() - HEAD_RADIUS*2;
		GLabel unguessedWord = new GLabel(word, x, y);
		unguessedWord.setFont("Times New Roman-30");
		
		//removes the latest hidden word and replaces it with the newest one with the new updated guessed letter
 ...
 */
	public void noteIncorrectGuess(char letter) {
		...
		GLabel incorrectLetters = new GLabel(wrongChars, x, y);
		...
		if(wrongChars.length() == 2) {
			drawBody();
		}
		if(wrongChars.length() == 3) {
			drawLeftArm();
		}
		if(wrongChars.length() == 4) {
			drawRightArm();
		}
		...
		if(wrongChars.length() == 7) {
			drawLeftFoot();
		}
		if(wrongChars.length() == 8) {
			drawRightFoot();
		}
	}
